<?php
    ...
    if (!isset($_SESSION['user_id'])) {
        ...
        exit;  }
    //feedback submission_________________________________________________________________________
    //checks if request method of $_SERVER is POST
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        //get $user_id from the user_id the session is established with
        $user_id = $_SESSION['user_id'];
        //get the feedback from user and store it in $comment
        $comment = $_POST['comment'];
        //insert into table feedback the user_id and comment
        //parameterized queries  to avoid sql injection
        $query = "INSERT INTO feedback (user_id, comment) VALUES (?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("is", $user_id, $comment);
        //if statement executed succesfully
        if ($stmt->execute()) {
            //success message
...
<html lang="en">
    ...
    <body>
        ...
        <nav class="nav nav-pills flex-column flex-sm-row">
            ...
            <a class="flex-sm-fill text-sm-center nav-link active" aria-current="page" href="feedback.php">Feedback</a>
        ...
        <!-- container for the feedback form ________________________________________________________________________-->
        <div class="container">
            <h2>Your Feedback Matters</h2>
            <form action="feedback.php" method="POST">
                <!-- the input feild for the feedback -->
                <textarea id ='comment' name="comment" placeholder="Your Feedback" required></textarea>
